...
- 详情灯箱：键盘 `←` `→` 切换、`ESC` 关闭、`#w128` 哈希直链分享；媒体框左上角可开启**作品原声**（默认静音）
- 全键盘可达：作品卡 `Tab` 聚焦、`Enter` 打开灯箱；灯箱内焦点圈禁、关闭归还焦点
- 中 / EN 双语切换：全站文案 + 作品标题/模型名，`localStorage` 记忆选择
- 声音开关：WebAudio 实时合成的暗厅氛围声（零音频素材），开声后打开作品有轻叮音
- 移动端汉堡菜单（≤760px），全屏暗厅导航
...
```ts
{
  ...
  model: "即梦 2.5", type: "video", ratio: "16:9",
